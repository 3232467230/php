...
class MMysql {
    ...
    public function __construct() {
        $conf = [
            'host'=>'hdm189315207.my3w.com',
            'port'=>3306,
            'user'=>'hdm189315207',
            'passwd'=>'666666asD',
            'dbname'=>'hdm189315207_db',
        ];
        //连接数据库
        // if ( is_null(self::$_dbh) ) {
        //     $this->_connect();
        // }
        $this->link=mysqli_connect($conf['host'], $conf['user'], $conf['passwd'],$conf['dbname']); //打开一个到 MySQL 服务器的新的连接：mysqli_connect("localhost","wrong_user","my_password","my_db");返回一个代表到 MySQL 服务器的连接的对象。
        if (!$this->link) {
    ...
    */
    ...
    public function getID($name,$value){
        $resd = $this->_doQueryone("SELECT * FROM demo1 WHERE $name = '$value' order by id desc");
        return $resd['id'];
    }
